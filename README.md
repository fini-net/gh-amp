...
to make sure that there are some CI failures every time.  We want CI failures
in this case so that the failure display functionality of `gh observer` is
being tested on every pull request.
